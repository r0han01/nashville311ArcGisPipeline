...
| GeoPackage & script | What it explains | Key highlights | Ready for GIS import ([⬇️ Download](#)) |
|---------------------|------------------|----------------|----------|
| `serviceEfficiency.gpkg` (`serviceEfficiency/serviceEfficiency.py`) | Where districts deliver the fastest responses relative to workload. | Efficiency score = workload ratio ÷ response-time ratio, quartiles, percentile ranks, request context for pop-ups. | [⬇️](https://nashville311-gis-analysis-data.s3.amazonaws.com/gpkg-public/serviceEfficiency/serviceEfficiency.gpkg) |
| `geographicPatterns.gpkg` (`geographicPatterns/geographicPatterns.py`) | Spatial density, clustering, concentration, variability. | Silhouette + Quantiles for categories, centroid-based stats, request distribution fields. | [⬇️](https://nashville311-gis-analysis-data.s3.amazonaws.com/gpkg-public/geographicPatterns/geographicPatterns.gpkg) |
| `temporalPatterns.gpkg` (`temporalPatterns/temporalPatterns.py`) | Weekday vs. weekend behavior by district. | Day-of-week breakdowns, weekday/weekend ratios, consistency measures, percentile labels. | [⬇️](https://nashville311-gis-analysis-data.s3.amazonaws.com/gpkg-public/temporalPatterns/temporalPatterns.gpkg) |
| `peakHourConsistency.gpkg` (`peakHourConsistency/peakHourConsistency.py`) | Peak vs. off-hour service demand. | Hour-of-day profiles, coefficient-of-variation metrics, Silhouette-based peak pattern types. | [⬇️](https://nashville311-gis-analysis-data.s3.amazonaws.com/gpkg-public/peakHourConsistency/peakHourConsistency.gpkg) |
| `requestVolatility.gpkg` (`requestVolatility/requestVolatility.py`) | Stability of request volumes. | Daily/weekly/monthly volatility coefficients, stability indexes, data-driven volatility categories. | [⬇️](https://nashville311-gis-analysis-data.s3.amazonaws.com/gpkg-public/requestVolatility/requestVolatility.gpkg) |
| `timeBasedClusters.gpkg` (`timeBasedClusters/timeBasedClusters.py`) | Temporal clustering of districts. | K-means with silhouette-selected k, cluster metrics (distance, confidence, dominant patterns). | [⬇️](https://nashville311-gis-analysis-data.s3.amazonaws.com/gpkg-public/timeBasedClusters/timeBasedClusters.gpkg) |
| `serviceTypeInsights.gpkg` (`serviceTypeInsights/serviceTypeInsights.py`) | Service mix, recurring issues, complexity. | Diversity/concentration (Herfindahl), recurring issue detection, Silhouette-based categories. | [⬇️](https://nashville311-gis-analysis-data.s3.amazonaws.com/gpkg-public/serviceTypeInsights/serviceTypeInsights.gpkg) |
| `trendDirection.gpkg` (`trendDirection/trendDirection.py`) | Increasing vs. decreasing request trends. | Monthly regression, slope/R²/p-value, projected next month, trend confidence scores. | [⬇️](https://nashville311-gis-analysis-data.s3.amazonaws.com/gpkg-public/trendDirection/trendDirection.gpkg) |
| `predictiveHotspots.gpkg` (`predictiveHotspots/predictiveHotspots.py`) | Future risk assessment based on multi-factor scoring. | Variance-based risk weights, dominant risk factors, percentile-based priority labels. | [⬇️](https://nashville311-gis-analysis-data.s3.amazonaws.com/gpkg-public/predictiveHotspots/predictiveHotspots.gpkg) |
| `capacityPlanning.gpkg` (`capacityPlanning/capacityPlanning.py`) | Who is at/near capacity and when they’ll hit limits. | Median + MAD + IQR thresholds, utilization & pressure, time-to-capacity, Silhouette categories. | [⬇️](https://nashville311-gis-analysis-data.s3.amazonaws.com/gpkg-public/capacityPlanning/capacityPlanning.gpkg) |
| `optimizationOpportunities.gpkg` (`optimizationOpportunities/optimizationOpportunities.py`) | Biggest improvement opportunities and expected impact. | Response/efficiency/workload gaps, volatility impact, variance-based opportunity scores, priority/category labels. | [⬇️](https://nashville311-gis-analysis-data.s3.amazonaws.com/gpkg-public/optimizationOpportunities/optimizationOpportunities.gpkg) |
